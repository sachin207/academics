...
if(isset($_SESSION['cart'])) {
  ...
  foreach($_SESSION['cart'] as $product_id => $quantity) {
    ...
    $result = $mysqli->query('SELECT * FROM camera_product WHERE prodId = "'.$product_id.'"');
    echo 'SELECT * FROM camera_product WHERE prodId = "'.$product_id.'"';
    if($result){
      ...
      if($obj = $result->fetch_object()) {
        ...
        $query = $mysqli->query("INSERT INTO orders (prodId, product_name, price, units, total, email) VALUES('$obj->prodId', '$obj->product_name', $obj->price, $quantity, $cost, '$user')");
        echo "INSERT INTO orders (prodId, product_name, price, units, total, email) VALUES('$obj->prodId', '$obj->product_name', $obj->price, $quantity, $cost, '$user')";
        if($query){
          $newqty = $obj->qty - $quantity;
          if($mysqli->query('UPDATE camera_product SET qty = '.$newqty.' WHERE prodId = "'.$product_id.'"')){
