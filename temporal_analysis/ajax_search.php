...
echo '<div class="small-12" style="margin-top:5px;">';
          ...
          { 
            ...
            {
              $query = 'SELECT * FROM camera_product WHERE product_name LIKE "%'.$search.'%"';
              $result = $mysqli->query($query);
              
            }
            else{
              $query = 'SELECT * FROM camera_product WHERE manufacturer = "'.$_SESSION["manufacturer"].'" AND  product_name LIKE "%'.$search.'%"';
              $result = $mysqli->query($query);
          ...
          else
          {
            if(strlen($_SESSION['manufacturer'])==0 || $_SESSION['manufacturer'] == "ALL")
            {
              $query = 'SELECT * FROM camera_product';
              $result = $mysqli->query($query);
              
            }
            else{
              $query = 'SELECT * FROM camera_product WHERE manufacturer = "'.$_SESSION["manufacturer"].'"';
              $result = $mysqli->query($query);
